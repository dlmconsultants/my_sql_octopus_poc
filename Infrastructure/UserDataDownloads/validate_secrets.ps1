...
if ($badSecretMessages -notlike ""){
    $errorMessage = "$errorMesage. Also, there are problems with the following secrets: $badSecretMessages"
}
if ($errorMessage -notlike ""){
    Update-StatupStatus -status "FAILED-AwsSecretsValidationErrors-SeeLogIn-C/Startup"
